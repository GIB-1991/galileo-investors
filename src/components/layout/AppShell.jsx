  ...
  return(
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',background:'var(--color-bg)'}}>
      ...
      <header style={{background:'rgba(13,15,20,0.9)',backdropFilter:'blur(20px)',borderBottom:'1px solid var(--color-border)',position:'sticky',top:0,zIndex:100}}>
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 1.5rem',height:62,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <Link to="/dashboard" style={{textDecoration:'none',display:'flex',alignItems:'center',gap:10}}>
            ...
            <div>
              <div style={{fontWeight:700,fontSize:'0.88rem',color:'var(--color-text-primary)'}}>משקיעים עם גלילאו</div>
              <div style={{fontSize:'0.6rem',color:'var(--color-accent)',letterSpacing:'0.12em',fontWeight:600,opacity:0.8}}>GALILEO INVESTORS</div>
            </div>
          </Link>
          ...
          <nav style={{display:'flex',alignItems:'center',gap:2}}>
            {NAV.map(item=>{
              ...
              return(
                <Link key={item.path} to={item.path} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:9,textDecoration:'none',fontSize:'0.85rem',fontWeight:active?600:500,color:active?'var(--color-accent)':'var(--color-text-secondary)',background:active?'rgba(245,166,35,0.1)':'transparent',border:active?'1px solid rgba(245,166,35,0.2)':'1px solid transparent',transition:'all 180ms'}}>
                  <Icon size={15}/>
                  {item.label}
                </Link>
              )
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            {daysLeft<=7&&(
              <div style={{display:'flex',alignItems:'center',gap:6,background:daysLeft<=3?'rgba(245,166,35,0.12)':'var(--color-surface)',border:`1px solid ${daysLeft<=3?'rgba(245,166,35,0.35)':'var(--color-border2)'}`,borderRadius:20,padding:'4px 12px',fontSize:'0.75rem',fontWeight:600,color:daysLeft<=3?'var(--color-accent)':'var(--color-text-secondary)'}}>
                <Clock size={12}/> {daysLeft} ימים נותרו
              </div>
            )}
            <span style={{fontSize:'0.78rem',color:'var(--color-text-muted)',background:'var(--color-surface)',padding:'4px 10px',borderRadius:8,border:'1px solid var(--color-border)'}}>
              {user?.email?.split('@')[0]}
            </span>
      ...
      {/* Content */}
      <main style={{flex:1,maxWidth:1280,margin:'0 auto',width:'100%',padding:'2rem 1.5rem'}}>
        {children}
      </main>
