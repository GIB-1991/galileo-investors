...
const MARKET=[
  {name:'S&P 500',value:'6,775.50',change:'+0.29%',up:true},
  {name:'Nasdaq 100',value:'25,047.75',change:'+0.13%',up:true},
  {name:'Gold',value:'$5,015',change:'+0.14%',up:true},
  {name:'USD/ILS',value:'3.68',change:'-0.22%',up:false},
]

const CAT_COLORS={
  'מאקרו':'rgba(79,142,247,0.15)',
  'טכנולוגיה':'rgba(168,85,247,0.15)',
  'שוק':'rgba(45,216,122,0.15)',
  'סחורות':'rgba(245,166,35,0.15)',
}
const CAT_TEXT={
  'מאקרו':'#60a5fa',
  'טכנולוגיה':'#c084fc',
  ...
  return(
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'1rem',marginBottom:'2rem'}}>
        {MARKET.map(m=>(
          <div key={m.name} style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:14,padding:'1.1rem 1.25rem',transition:'border-color 200ms'}}
            onMouseEnter={e=>e.currentTarget.style.borderColor='var(--color-border2)'}
            onMouseLeave={e=>e.currentTarget.style.borderColor='var(--color-border)'}>
            <div style={{fontSize:'0.75rem',color:'var(--color-text-muted)',marginBottom:6,fontWeight:600,letterSpacing:'0.04em'}}>{m.name}</div>
            <div style={{fontSize:'1.2rem',fontWeight:800,direction:'ltr',textAlign:'right',fontFamily:"'IBM Plex Mono',monospace",color:'var(--color-text-primary)',marginBottom:4}}>{m.value}</div>
            <div style={{display:'flex',alignItems:'center',gap:4,justifyContent:'flex-end'}}>
              {m.up?<TrendingUp size={13} style={{color:'var(--color-success)'}}/>:<TrendingDown size={13} style={{color:'var(--color-danger)'}}/>}
              <span style={{fontSize:'0.8rem',fontWeight:700,color:m.up?'var(--color-success)':'var(--color-danger)',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace"}}>{m.change}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:14,overflow:'hidden'}}>
        <div style={{padding:'1.1rem 1.5rem',borderBottom:'1px solid var(--color-border)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <button onClick={()=>{setLoading(true);setTimeout(()=>setLoading(false),600)}} style={{background:'none',border:'none',cursor:'pointer',color:'var(--color-text-muted)',display:'flex',alignItems:'center',gap:4,fontSize:'0.75rem',padding:'4px 8px',borderRadius:6,transition:'all 180ms'}}
        ...
        {loading?(
          ...
          <div>
            {news.map((item,i)=>(
              <div key={item.id} style={{padding:'0.95rem 1.5rem',borderBottom:i<news.length-1?'1px solid var(--color-border)':'none',display:'flex',alignItems:'flex-start',gap:'1rem',cursor:'pointer',transition:'background 150ms'}}
