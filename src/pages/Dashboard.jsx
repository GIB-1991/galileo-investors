          ...
          <div>{news.map((item, i) => (
            <a key={item.id} href={item.url} target='_blank' rel='noopener noreferrer'
              ...
              <div style={{flex:1}}>
                ...
                {item.tickers&&item.tickers.length>0&&<div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:5}}>{item.tickers.map(t=><span key={t} style={{fontSize:'.68rem',fontWeight:700,padding:'2px 7px',borderRadius:5,background:'rgba(79,142,247,0.12)',color:'#4f8ef7',border:'1px solid rgba(79,142,247,0.25)',fontFamily:"'IBM Plex Mono',monospace"}}>{'$'+t}</span>)}</div>}
              </div>
            </a>
          ))}</div>
        )}
