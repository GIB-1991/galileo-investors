  ...
  return (
    <div>
      <div style={{marginBottom:'1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        ...
        {sectorPieData.length>0&&(<div style={{background:'var(--color-surface)',borderRadius:'1rem',padding:'1.25rem',marginBottom:'1.5rem'}}>
          ...
          <div style={{display:'flex',gap:'1.5rem',alignItems:'center',flexWrap:'wrap'}}>
            <ResponsiveContainer width={200} height={200}><PieChart>
              ...
              <Tooltip content={({active,payload})=>{
                if(!active||!payload?.length) return null
                const d=payload[0].payload
                return(<div style={{background:'var(--color-surface-2)',border:'1px solid var(--color-border)',borderRadius:8,padding:'8px 12px',fontSize:'.82rem',direction:'rtl'}}>
                  <div style={{fontWeight:700}}>{d.name}</div>
                  <div style={{color:'var(--color-text-muted)'}}>{d.pct}%</div>
                </div>)
              }}/>
