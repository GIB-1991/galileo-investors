...
const SK = 'galileo_portfolio_v2'
const HK = 'galileo_history_v1'
const loadP = () => { try { return JSON.parse(localStorage.getItem(SK))||[] } catch { return [] } }
const saveP = d => localStorage.setItem(SK, JSON.stringify(d))
const loadH = () => { try { return JSON.parse(localStorage.getItem(HK))||[] } catch { return [] } }
const saveH = d => localStorage.setItem(HK, JSON.stringify(d))

export default function Portfolio() {
  const [holdings, setHoldings] = useState(loadP)
  ...
  const addHolding = () => {
    if (!sel||!shares||!buyPrice) return
    const n = { id:Date.now(), ticker:sel.ticker, name:sel.name, shares:parseFloat(shares), buyPrice:parseFloat(buyPrice) }
    const upd = [...holdings, n]; setHoldings(upd); saveP(upd)
    const e = { id:Date.now()+1, type:'buy', ticker:n.ticker, name:n.name, shares:n.shares, price:n.buyPrice, date:new Date().toLocaleDateString('he-IL'), ts:Date.now() }
    const uh = [e, ...history]; setHistory(uh); saveH(uh)
    setQuery(''); setSel(null); setShares(''); setBuyPrice(''); setShowAdd(false)
    refreshPrices(upd)
  }

  const removeHolding = (id) => {
    const upd = holdings.filter(h=>h.id!==id); setHoldings(upd); saveP(upd)
  }
  ...
  const confirmSell = () => {
    ...
    const e = { id:Date.now(), type:'sell', ticker:showSell.ticker, name:showSell.name, shares:ss, price:sp, buyPrice:showSell.buyPrice, date:new Date().toLocaleDateString('he-IL'), ts:Date.now() }
    const uh = [e,...history]; setHistory(uh); saveH(uh)
    const upd = ss>=showSell.shares ? holdings.filter(h=>h.id!==showSell.id) : holdings.map(h=>h.id===showSell.id?{...h,shares:h.shares-ss}:h)
    setHoldings(upd); saveP(upd)
  ...
  return (
    <div>
      <div style={{marginBottom:'1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div><h1 style={{fontSize:'1.5rem',fontWeight:800,margin:'0 0 4px'}}>תיק השקעות</h1><p style={{color:'var(--color-text-muted)',margin:0,fontSize:'.875rem'}}>מעקב, ניתוח ומסחר</p></div>
        <div style={{display:'flex',gap:8}}>
          ...
          {tab==='portfolio'&&<button className="btn-primary" onClick={()=>setShowAdd(true)} style={{display:'flex',alignItems:'center',gap:6}}><Plus size={14}/> הוסף</button>}
        </div>
      </div>
      {tab==='portfolio' && (<div>
        ...
        {enriched.length>0 ? (<div className="card" style={{padding:0,overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:'.875rem'}}><thead><tr style={{background:'var(--color-bg2)',borderBottom:'1px solid var(--color-border)'}}>{['Ticker','שם','מחיר','קנייה','מניות','שווי','P&L','%',''].map(h=>(<th key={h} style={{padding:'.7rem .875rem',textAlign:'right',fontWeight:600,fontSize:'.75rem',color:'var(--color-text-muted)',whiteSpace:'nowrap'}}>{h}</th>))}</tr></thead><tbody>{enriched.map(row=>(<tr key={row.id} style={{borderBottom:'1px solid var(--color-border)'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><td style={{padding:'.8rem .875rem'}}><span className="ticker-badge">{row.ticker}</span></td><td style={{padding:'.8rem .875rem',color:'var(--color-text-secondary)',fontSize:'.8rem',maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',direction:'ltr'}}>{row.name||row.ticker}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",fontWeight:600}}>${row.cur.toFixed(2)}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",color:'var(--color-text-muted)',fontSize:'.8rem'}}>${row.buyPrice.toFixed(2)}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace"}}>{row.shares}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",fontWeight:600}}>${row.val.toLocaleString('en-US',{maximumFractionDigits:0})}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",color:row.pnl>=0?'var(--color-success)':'var(--color-danger)',fontWeight:600}}>{fm(row.pnl)}</td><td style={{padding:'.8rem .875rem'}}><span style={{color:row.pnlPct>=0?'var(--color-success)':'var(--color-danger)',fontWeight:600,direction:'ltr',display:'flex',alignItems:'center',gap:3}}>{row.pnlPct>=0?<TrendingUp size={11}/>:<TrendingDown size={11}/>}{Math.abs(row.pnlPct).toFixed(1)}%</span></td><td style={{padding:'.8rem .875rem'}}><div style={{display:'flex',gap:5}}><button onClick={()=>openSell(row)} style={{padding:'3px 10px',borderRadius:6,background:'rgba(245,166,35,0.1)',border:'1px solid rgba(245,166,35,0.3)',cursor:'pointer',color:'var(--color-accent)',fontSize:'.75rem',fontWeight:700,fontFamily:'Heebo,sans-serif'}}>מכור</button><button onClick={()=>removeHolding(row.id)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--color-text-muted)',padding:3}} onMouseEnter={e=>e.currentTarget.style.color='var(--color-danger)'} onMouseLeave={e=>e.currentTarget.style.color='var(--color-text-muted)'}><Trash2 size={13}/></button></div></td></tr>))}</tbody></table></div>) : (<div style={{textAlign:'center',padding:'4rem',color:'var(--color-text-muted)'}}><div style={{fontSize:'3rem',marginBottom:'1rem'}}>📊</div><p style={{margin:'0 0 1rem'}}>התיק ריק. הוסף מניה ראשונה!</p><button className="btn-primary" onClick={()=>setShowAdd(true)} style={{display:'inline-flex',alignItems:'center',gap:6}}><Plus size={14}/> הוסף מניה</button></div>)}
      </div>)}
      {tab==='history' && (history.length===0 ? (<div style={{textAlign:'center',padding:'4rem',color:'var(--color-text-muted)'}}><History size={40} style={{marginBottom:'1rem',opacity:.3}}/><p style={{margin:0}}>עדיין אין פעולות</p></div>) : (<div className="card" style={{padding:0,overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:'.875rem'}}><thead><tr style={{background:'var(--color-bg2)',borderBottom:'1px solid var(--color-border)'}}>{['סוג','Ticker','שם','מניות','מחיר','שווי','רווח/הפסד','תאריך'].map(h=>(<th key={h} style={{padding:'.7rem .875rem',textAlign:'right',fontWeight:600,fontSize:'.75rem',color:'var(--color-text-muted)',whiteSpace:'nowrap'}}>{h}</th>))}</tr></thead><tbody>{history.map(item=>{const isSell=item.type==='sell',val=item.price*item.shares,pnl=isSell?(item.price-(item.buyPrice||0))*item.shares:null;return(<tr key={item.id} style={{borderBottom:'1px solid var(--color-border)'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><td style={{padding:'.8rem .875rem'}}><span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'3px 10px',borderRadius:20,fontSize:'.72rem',fontWeight:700,background:isSell?'rgba(240,82,82,0.12)':'rgba(45,216,122,0.12)',color:isSell?'var(--color-danger)':'var(--color-success)'}}>{isSell?<ArrowUpCircle size={11}/>:<ArrowDownCircle size={11}/>}{isSell?'מכירה':'קנייה'}</span></td><td style={{padding:'.8rem .875rem'}}><span className="ticker-badge">{item.ticker}</span></td><td style={{padding:'.8rem .875rem',color:'var(--color-text-secondary)',fontSize:'.8rem',direction:'ltr',maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{item.name||item.ticker}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace"}}>{item.shares}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace"}}>{'$'+item.price.toFixed(2)}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",fontWeight:600}}>{'$'+val.toLocaleString('en-US',{maximumFractionDigits:0})}</td><td style={{padding:'.8rem .875rem',direction:'ltr',fontFamily:"'IBM Plex Mono',monospace",color:pnl==null?'var(--color-text-muted)':pnl>=0?'var(--color-success)':'var(--color-danger)',fontWeight:pnl!=null?700:400}}>{pnl!=null?((pnl>=0?'+':'-')+'$'+Math.abs(pnl).toFixed(0)):'—'}</td><td style={{padding:'.8rem .875rem',color:'var(--color-text-muted)',fontSize:'.8rem'}}>{item.date}</td></tr>)})}</tbody></table></div>))}
      {showAdd&&(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:'1rem'}} onClick={e=>{if(e.target===e.currentTarget)setShowAdd(false)}}><div style={{background:'var(--color-surface)',border:'1px solid var(--color-border)',borderRadius:16,padding:'2rem',width:'100%',maxWidth:420,boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}><h3 style={{margin:0,fontWeight:700}}>הוסף מניה</h3><button onClick={()=>setShowAdd(false)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--color-text-muted)'}}><X size={17}/></button></div><div style={{display:'flex',flexDirection:'column',gap:'1rem'}}><div><label style={{fontSize:'.8rem',fontWeight:600,display:'block',marginBottom:6,color:'var(--color-text-secondary)'}}>חיפוש מניה</label><div style={{position:'relative'}}><Search size={13} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',color:'var(--color-text-muted)'}}/><input className="input input-ticker" value={query} onChange={e=>{setQuery(e.target.value.toUpperCase());setSel(null)}} onFocus={()=>suggestions.length>0&&setShowSugg(true)} onBlur={()=>setTimeout(()=>setShowSugg(false),200)} placeholder="AAPL, Tesla..." style={{paddingRight:34}} autoComplete="off"/>{showSugg&&suggestions.length>0&&(<div style={{position:'absolute',top:'calc(100% + 4px)',right:0,left:0,background:'var(--color-surface)',border:'1px solid var(--color-border2)',borderRadius:10,zIndex:200,overflow:'hidden',boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>{suggestions.map(s=>(<div key={s.ticker} onMouseDown={()=>pickTicker(s.ticker,s.name)} style={{padding:'.6rem 1rem',cursor:'pointer',display:'flex',alignItems:'center',gap:10}} onMouseEnter={e=>e.currentTarget.style.background='var(--color-bg2)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontWeight:700,fontSize:'.8rem',color:'var(--color-accent)',minWidth:52}}>{s.ticker}</span><span style={{fontSize:'.8rem',color:'var(--color-text-secondary)'}}>{(s.name||'').substring(0,28)}</span></div>))}</div>)}</div>{sel&&<div style={{marginTop:4,fontSize:'.75rem',color:'var(--color-success)'}}>&#10003; {sel.name}</div>}</div><div><label style={{fontSize:'.8rem',fontWeight:600,display:'block',marginBottom:6,color:'var(--color-text-secondary)'}}>מניות</label><input className="input" type="number" value={shares} onChange={e=>setShares(e.target.value)} placeholder="10" style={{direction:'ltr'}}/></div><div><label style={{fontSize:'.8rem',fontWeight:600,display:'block',marginBottom:6,color:'var(--color-text-secondary)'}}>מחיר קנייה ($) {priceLoading&&<span style={{color:'var(--color-accent)',fontSize:'.72rem'}}>טוען...</span>}</label><input className="input" type="number" value={buyPrice} onChange={e=>setBuyPrice(e.target.value)} placeholder="מחיר שוק" style={{direction:'ltr'}}/></div><button className="btn-accent" onClick={addHolding} disabled={!sel||!shares||!buyPrice} style={{opacity:(!sel||!shares||!buyPrice)?0.5:1}}>הוסף לתיק</button></div></div></div>)}
