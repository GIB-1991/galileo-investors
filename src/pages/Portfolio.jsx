...
const COLORS = ['var(--color-accent)','var(--color-info)','var(--color-success)','#8E7CC3','var(--color-danger)','#4FA8C4','var(--color-accent)','#34d399','#818cf8','#fb7185']
    ...
    <div>
      <div style={{marginBottom:'1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {['portfolio','history'].map(t=>(<button key={t} onClick={()=>setTab(t)} style={{padding:'7px 16px',borderRadius:9,border:'1px solid',cursor:'pointer',fontFamily:'Heebo,sans-serif',fontWeight:600,fontSize:'.85rem',background:tab===t?'var(--color-accent)':'transparent',color:tab===t?'var(--color-on-accent)':'var(--color-text-secondary)',borderColor:tab===t?'var(--color-accent)':'var(--color-border2)'}}>{t==='portfolio'?'תיק':'היסטוריה'}</button>))}
          {tab==='portfolio'&&<button onClick={()=>setShowAdd(true)} style={{display:'flex',alignItems:'center',gap:8,background:'linear-gradient(135deg,var(--color-accent) 0%,var(--color-accent2) 100%)',color:'#fff',border:'none',borderRadius:'0.6rem',padding:'0.55rem 1.1rem',fontSize:'0.9rem',fontWeight:700,cursor:'pointer',boxShadow:'0 2px 8px rgba(212,175,55,0.35)',transition:'transform .15s,box-shadow .15s',direction:'rtl'}} onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.05)';e.currentTarget.style.boxShadow='0 4px 14px rgba(212,175,55,0.5)'}} onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)';e.currentTarget.style.boxShadow='0 2px 8px rgba(212,175,55,0.35)'}}><Plus size={16}/> הוסף מניה</button>}
        ...
        {tips.length>0&&(<div style={{marginBottom:'1.5rem',display:'flex',flexDirection:'column',gap:'.5rem'}}>{tips.map((a,i)=>(<div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'.875rem 1rem',borderRadius:10,border:'1px solid',background:a.type==='danger'?'rgba(224,102,107,0.08)':a.type==='warning'?'rgba(212,175,55,0.08)':'rgba(63,185,129,0.08)',borderColor:a.type==='danger'?'rgba(224,102,107,0.25)':a.type==='warning'?'rgba(212,175,55,0.25)':'rgba(63,185,129,0.25)'}}><AlertTriangle size={14} style={{color:a.type==='danger'?'var(--color-danger)':a.type==='warning'?'var(--color-warning)':'var(--color-success)',flexShrink:0,marginTop:2}}/><span style={{fontSize:'.9rem',color:'var(--color-text-primary)',fontWeight:'700',lineHeight:1.4}}>{a.message}</span></div>))}</div>)}
        {enriched.length>0 ? (
              ...
              <tbody>{enriched.map(row=>(
                <tr key={row.id} style={{borderBottom:'1px solid var(--color-border)'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                  ...
                  <td style={{padding:'.8rem .875rem'}}><div style={{display:'flex',gap:5}}>
                    <button onClick={()=>openSell(row)} style={{padding:'3px 10px',borderRadius:6,background:'rgba(212,175,55,0.1)',border:'1px solid rgba(212,175,55,0.3)',cursor:'pointer',color:'var(--color-accent)',fontSize:'.75rem',fontWeight:700,fontFamily:'Heebo,sans-serif'}}>מכור</button>
          ...
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:'.875rem'}}>
            ...
            <tbody>{history.map(item=>{
              ...
              return (<tr key={item.id} style={{borderBottom:'1px solid var(--color-border)'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                <td style={{padding:'.8rem .875rem'}}><span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'3px 10px',borderRadius:20,fontSize:'.72rem',fontWeight:700,background:isSell?'rgba(224,102,107,0.12)':'rgba(63,185,129,0.12)',color:isSell?'var(--color-danger)':'var(--color-success)'}}>{isSell?<ArrowUpCircle size={11}/>:<ArrowDownCircle size={11}/>}{isSell?'מכירה':'קנייה'}</span></td>
...
function SellPnL({ sp, bp, ss }) {
  const p = (sp - bp) * ss
  return (<div style={{background:p>=0?'rgba(63,185,129,0.1)':'rgba(224,102,107,0.1)',borderRadius:9,padding:'.75rem',textAlign:'center',border:'1px solid',borderColor:p>=0?'rgba(63,185,129,0.25)':'rgba(224,102,107,0.25)',fontSize:'.85rem'}}>רווח/הפסד: <strong style={{color:p>=0?'var(--color-success)':'var(--color-danger)',direction:'ltr',display:'inline-block'}}>{p>=0?'+':'-'}{'$'+Math.abs(p).toFixed(2)}</strong></div>)
}