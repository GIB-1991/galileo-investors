...
export default function Admin({ user }) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      {msg && <div style={{ padding: '10px 14px', marginBottom: '1rem', background: 'rgba(63,185,129,.1)', border: '1px solid rgba(63,185,129,.3)', borderRadius: 8, color: 'var(--color-success)', fontSize: '.9rem' }}>{msg}</div>}

      <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem', borderBottom: '1px solid var(--color-border)' }}>
        <button onClick={() => setTab('articles')} style={{ padding: '10px 18px', background: 'transparent', border: 'none', borderBottom: tab==='articles' ? '2px solid var(--color-accent)' : '2px solid transparent', color: tab==='articles' ? 'var(--color-accent)' : 'var(--color-text-secondary)', cursor: 'pointer', fontWeight: 600, fontSize: '.9rem' }}>
          <FileText size={14} style={{marginLeft:6}}/> מאמרים ({articles.length})
        </button>
        <button onClick={() => setTab('academy')} style={{ padding: '10px 18px', background: 'transparent', border: 'none', borderBottom: tab==='academy' ? '2px solid var(--color-accent)' : '2px solid transparent', color: tab==='academy' ? 'var(--color-accent)' : 'var(--color-text-secondary)', cursor: 'pointer', fontWeight: 600, fontSize: '.9rem' }}>
          <BookOpen size={14} style={{marginLeft:6}}/> אקדמיה ({academy.length})
        </button>
        <button onClick={() => setTab('users')} style={{ padding: '10px 18px', background: 'transparent', border: 'none', borderBottom: tab==='users' ? '2px solid var(--color-accent)' : '2px solid transparent', color: tab==='users' ? 'var(--color-accent)' : 'var(--color-text-secondary)', cursor: 'pointer', fontWeight: 600, fontSize: '.9rem' }}>
      ...
      {tab === 'articles' && (
        <div>
          <button onClick={() => setEditingArticle({ title:'', summary:'', content:'', image_url:'', url:'', category:'כללי', published:false })}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'10px 18px', borderRadius:9, border:'none', background:'var(--color-accent)', color:'var(--color-on-accent)', cursor:'pointer', fontWeight:700, fontSize:'.9rem', marginBottom:'1rem' }}>
            ...
            {articles.map(a => (
              <div key={a.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:10 }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, flex:1, minWidth:0 }}>
                  <span style={{ fontSize:'.7rem', padding:'3px 8px', borderRadius:4, fontWeight:700, background: a.published ? 'rgba(63,185,129,.15)' : 'rgba(212,175,55,.15)', color: a.published ? 'var(--color-success)' : 'var(--color-accent)' }}>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  ...
                  <button onClick={() => deleteArticle(a.id)} style={{ padding:'6px 10px', borderRadius:7, border:'1px solid rgba(224,102,107,.3)', background:'transparent', color:'var(--color-danger)', cursor:'pointer' }}><Trash2 size={13}/></button>
                </div>
              </div>
            ))}
            ...
            {academy.map(a => (
              <div key={a.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:10 }}>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  ...
                  <button onClick={() => deleteAcademy(a.id)} style={{ padding:'6px 10px', borderRadius:7, border:'1px solid rgba(224,102,107,.3)', background:'transparent', color:'var(--color-danger)', cursor:'pointer' }}><Trash2 size={13}/></button>
                </div>
              </div>
            ))}
          </div>
        ...
        {showPreview && (
          <div style={{ flex:'1 1 50%', overflow:'auto', padding:'2rem 2.5rem', background:'var(--color-bg)' }}>
            <div style={{ maxWidth:680, margin:'0 auto' }}>
              <div style={{ fontSize:'.7rem', fontWeight:700, color:'var(--color-accent)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>תצוגה מקדימה</div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 24px', borderTop:'1px solid var(--color-border)', background:'var(--color-surface)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <span style={{ fontSize:'.85rem', color:'var(--color-text-muted)' }}>
            {form.id ? 'עריכת מאמר' : 'מאמר חדש'} · {form.published ? <span style={{color:'var(--color-success)',fontWeight:600}}>מפורסם</span> : <span style={{color:'var(--color-accent)',fontWeight:600}}>טיוטה</span>}
          </span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          {msg && <span style={{ fontSize:'.8rem', color:'var(--color-success)', fontWeight:600 }}>{msg}</span>}
          ...
          <button onClick={() => handleSave(true)} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 18px', borderRadius:8, border:'none', background:'var(--color-accent)', color:'var(--color-on-accent)', cursor:'pointer', fontWeight:700, fontSize:'.88rem' }}>
...
function AcademyForm({ data, onSave, onCancel }) {
  ...
  return (
    <div style={{ background:'var(--color-surface)', border:'1px solid rgba(212,175,55,0.2)', borderRadius:12, padding:'1.5rem', marginBottom:'1rem' }}>
      <h3 style={{ margin:'0 0 1rem', color:'var(--color-accent)', fontWeight:700 }}>{form.id ? 'עריכת פריט' : 'פריט חדש'}</h3>
      ...
      <div style={{ display:'flex', gap:10 }}>
        <button onClick={()=>onSave(form)} style={{ display:'flex', alignItems:'center', gap:5, padding:'8px 16px', borderRadius:8, border:'none', background:'var(--color-accent)', color:'var(--color-on-accent)', cursor:'pointer', fontWeight:700, fontSize:'.85rem' }}><Save size={13}/> שמור</button>
