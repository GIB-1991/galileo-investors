...
// ---- Fallback data (used only if the live API is unavailable) ----
const FB_QUARTER='Q4 2025'
const FB_REPORT_DATE='31 Dec 2025'
const FB_FILING_DATE='Feb 2026'

const FB_INV=[
{id:'BRK',name:'Warren Buffett',fund:'Berkshire Hathaway',img:'/api/imgproxy?url=https%3A%2F%2Fupload.wikimedia.org%2Fwikipedia%2Fcommons%2Fthumb%2Fd%2Fd4%2FWarren_Buffett_at_the_2015_SelectUSA_Investment_Summit_%2528cropped%2529.jpg%2F330px-Warren_Buffett_at_the_2015_SelectUSA_Investment_Summit_%2528cropped%2529.jpg',aum:298000,style:'Value',n:44,color:'var(--color-accent)',
...
{id:'PS',name:'Bill Ackman',fund:'Pershing Square',img:'https://upload.wikimedia.org/wikipedia/commons/thumb/0/07/Valeant_Pharmaceuticals%27_Business_Model_%28headshot%29.jpg/250px-Valeant_Pharmaceuticals%27_Business_Model_%28headshot%29.jpg',aum:11400,style:'Activist',n:11,color:'var(--color-info)',
...
{id:'SC',name:'Michael Burry',fund:'Scion Asset Management',img:'https://hihmkuaxnizvufwrtmgm.supabase.co/storage/v1/object/public/investors/rJvBIAyOK_0_175_3000_1688_0_x-large.jpg',aum:114,style:'Contrarian',n:9,color:'var(--color-danger)',
...
const FB_SECTORS=[
{name:'טכנולוגיה',value:41,color:'var(--color-info)'},{name:'פיננסים',value:17,color:'var(--color-accent)'},
{name:'בריאות',value:11,color:'var(--color-success)'},{name:'צריכה',value:9,color:'#8E7CC3'},
{name:'תעשייה',value:8,color:'var(--color-danger)'},{name:'אנרגיה',value:6,color:'#4FA8C4'},
]
...
.then(d=>{ if(alive && d && Array.isArray(d.investors) && d.investors.length) setData(d) })
.catch(()=>{})
...
const buys = live ? data.stats.buys : 321
const sells = live ? data.stats.sells : 196
const holdingsCount = live ? data.stats.holdings : 215
const leadSector = live && data.stats.leadSector ? data.stats.leadSector : 'Tech'
const leadPct = live ? data.stats.leadSectorPct : 41

const ACTIVITY=[
{q:QUARTER,קניות:buys,מכירות:sells},
]
...
<Circle label='סה"כ AUM מנוהל' value={fm(totalAUM*1e6)} color='var(--color-accent)'/>
<Circle label='Superinvestors במעקב' value={String(INV.length)} sub='מנהלים' color='var(--color-info)'/>
<Circle label='אחזקות מניות' value={String(holdingsCount)} sub={QUARTER} color='var(--color-success)'/>
<Circle label='קניות ברבעון' value={String(buys)} sub={'vs '+sells+' מכירות'} color='#8E7CC3'/>
<Circle label='ביצוע ממוצע 5Y' value='+24%' sub='vs S&P +13%' color='var(--color-danger)'/>
...
<img src={inv.img} alt={inv.name} onError={e=>{e.target.src='https://ui-avatars.com/api/?name='+encodeURIComponent(e.target.alt||'?')+'&size=120&background=555&color=fff&bold=true'}}
...
<div style={{fontFamily:"'IBM Plex Mono',monospace",fontWeight:800,color:inv.color,fontSize:'.85rem'}}>{fm((inv.aum||0)*1e6)}</div>
...
<img src={'https://logo.clearbit.com/'+h.t.toLowerCase().replace(/[^a-z]/g,'')+'.com'} alt={h.t} onError={e=>{e.target.src='https://ui-avatars.com/api/?name='+encodeURIComponent(e.target.alt||'?')+'&size=120&background=555&color=fff&bold=true'}} style={{width:18,height:18,borderRadius:3,objectFit:'contain',background:'white',padding:1,flexShrink:0}}/>
...
<div className="card" style={{marginBottom:'1.5rem',padding:'1.5rem'}}>
<h2 style={{margin:'0 0 1rem',fontSize:'.95rem',fontWeight:700,display:'flex',alignItems:'center',gap:8}}>
<TrendingUp size={15} style={{color:'var(--color-success)'}}/> המניות הכי מושקעות — {QUARTER}
</h2>
...
{['#','','מניה','שם','משקיעים','% ממוצע','שינוי'].map(h=>(
...
<img src={s.logo} alt={s.t} onError={e=>{e.target.src='https://ui-avatars.com/api/?name='+encodeURIComponent(e.target.alt||'?')+'&size=120&background=555&color=fff&bold=true'}}
...
<div style={{height:5,width:Math.round(s.inv*8),background:'var(--color-accent)',borderRadius:3,opacity:.7}}/>
...
{SECTORS.map((s,i)=><Cell key={i} fill={s.color||'var(--color-info)'}/>)}
</Pie><Tooltip formatter={v=>v+'%'}/></PieChart>
</ResponsiveContainer>
<div style={{flex:1,minWidth:100}}>
{SECTORS.map(s=>(
<div key={s.name} style={{display:'flex',alignItems:'center',gap:7,marginBottom:5}}>
<div style={{width:9,height:9,borderRadius:2,background:s.color||'var(--color-info)',flexShrink:0}}/>
...
<div className="card" style={{padding:'1.5rem'}}>
<h2 style={{margin:'0 0 1rem',fontSize:'.95rem',fontWeight:700}}>פעילות קניות/מכירות — {QUARTER}</h2>
...
<Bar dataKey="קניות" fill="var(--color-success)" radius={[3,3,0,0]}/>
<Bar dataKey="מכירות" fill="var(--color-danger)" radius={[3,3,0,0]}/>
</BarChart>
</ResponsiveContainer>
</div>
</div>
